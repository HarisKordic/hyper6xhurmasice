...
export default function EcoMap() {
   ...
   <Card className="absolute top-4 right-4 md:w-80 bg-black/80 backdrop-blur-md border-zinc-800 z-50 shadow-lg p-4">
    <ScrollArea className="h-48 rounded-lg">
     {Array.from({ length: 3 }).map((_, index) => (
      <div key={index} className="flex items-start space-x-4 mb-4 p-2 rounded-lg bg-zinc-800/30 animate-pulse">
 ...
 return (
  <div className="h-screen w-full bg-black text-white relative">
     ...
     {users?.map((user) => (
      <div
       ...
       className={`flex space-x-4 mb-4 p-4 rounded-lg transition-colors items-center cursor-pointer ${
        selectedUser?.id === user.id ? "shadow-[1px_1px_10px_rgba(0,0,0,0.13)]" : "hover:bg-zinc-800/30"
       }`}
       onClick={() => {
      ...
      >
       ...
       <div className="flex-1">
        <div className="flex items-center justify-between">
         ...
         <span className="text-xs text-black">{new Date(user.timestamp).toLocaleTimeString()}</span>
        </div>
        <p className="text-sm text-black">{user.activity}</p>
        <div className="flex items-center mt-1">
         {getIcon(user.iconType)}
         <span className="text-xs text-green-500 ml-1">{user.emission ? `-${user.emission}kg CO2` : "0kg CO2"}</span>
        </div>
       </div>
      </div>
     ))}
   ...
   >
    {users?.map((user) => (
     <Marker key={user.id} longitude={user.longitude} latitude={user.latitude} anchor="bottom">
      <div className="relative group cursor-pointer" onClick={() => setSelectedUser(user)}>
