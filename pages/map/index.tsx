     ...
     {users?.map((user) => (
      <div
       ...
       className={`flex items-start space-x-4 mb-4 p-2 rounded-lg transition-colors ${
        selectedUser?.id === user.id ? "bg-zinc-800/50" : "hover:bg-zinc-800/30"
       }`}
      >
       ...
       <div className="flex-1">
        <div className="flex items-center justify-between">
         ...
         <span className="text-xs text-zinc-400">{new Date(user.timestamp).toLocaleTimeString()}</span>
        </div>
   ...
   <div className="h-screen w-full bg-zinc-900 overflow-hidden relative">
    ...
    <div className="absolute inset-0 pointer-events-none">
     {users?.map((user) => (
      ...
      >
       <div className="relative group cursor-pointer pointer-events-auto" onClick={() => setSelectedUser(user)}>
